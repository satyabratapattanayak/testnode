...
const Model = require('./subIndustry.model');
...
const create = async (req, res, next) => {
    try {
        ...
        const result = await Model.create(req.body, CurrentUserInfo);
...
const update = async (req, res, next) => {
    try {
        ...
        const result = await Model.update(req.params.id, req.body, CurrentUserInfo);
        if (result === 0) {
