...
const Model = require('./recemmondationChart.model');
const { getCurrentUserInfo } = require('../shared/shared.controller');

const listAll = async (req, res, next) => {
    await Model.listAll().then((result) => {
...
const Details = async (req, res, next) => {
    ...
    await Model.details(id).then((result) => {
        if (isEmpty(result)) {
            res.status(404).json({ message: 'No data found', status: 404 });
        } else {
            if (result[0].file && result[0].file.length > 0) {
                result[0].file.forEach((ele) => {
