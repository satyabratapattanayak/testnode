...
const collection_name = 'recommendationChart';
let mydb;
...
const model = {
    ...
    create: async (reqBody, loggedUser) => {
        try {
            ...
            if (reqBody.file && reqBody.file.length > 0) {
                let arr = [];
                reqBody.file.forEach((ele) => {
                    arr.push(ele._id);
                });
                reqBody.file = arr;
            }
            ...
            saveActivity({
                ...
                data: newData,
                message: 'created a Recemmondation Chart'
            });
            return newData;
        } catch (error) {
            throw new Error(error);
    ...
    update: async (id, reqBody, loggedUser) => {
        try {
            ...
            saveActivity({
                ...
                userId: ObjectId(loggedUser._id),
                data: reqBody,
                message: 'updated a Recemmondation Chart'
            });
    ...
    delete: async (id, reqBody, loggedUser) => {
        try {
            ...
            saveActivity({
                ...
                userId: ObjectId(loggedUser._id),
                data: reqBody,
                message: 'deleted a Recemmondation Chart'
            });
            return result.matchedCount > 0 ? 1 : 0;
        } catch (error) {
            throw new Error(error);
